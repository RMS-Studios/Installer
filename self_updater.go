...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/RMS-Studios/Installer/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "RMSInstallerCli.exe", "RMSInstaller.exe")
		return BaseUrl + filename
	case "darwin":
		return BaseUrl + "RMSInstaller.MacOS.zip"
	case "linux":
		return BaseUrl + "RMSInstallerCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "RMSInstallerUpdate")
	if err != nil {
