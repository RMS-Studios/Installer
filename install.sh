...
if grep -q "CHROMEOS_RELEASE_NAME" /etc/lsb-release 2>/dev/null; then
	echo "ChromeOS is not supported! See https://github.com/zxkuhl/RMS for alternatives."
	exit 1
fi
...
curl -sS https://github.com/RMS-Studios/Installer/releases/latest/download/RMSInstallerCli-Linux \
  --output "$outfile" \
  --location \
  --fail
