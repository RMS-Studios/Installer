...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install RMS")
	var updateFlag = flag.Bool("repair", false, "Repair RMS")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall RMS")
	...
	if *versionFlag {
		fmt.Println("RMS Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2023 zxkuhl and RMS contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update RMS Installer' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest RMS files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, RMS will likely not work.")
}
