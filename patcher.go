...
func init() {
	if dir := os.Getenv("RMS_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using RMS_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../RMSData")
		BaseDir = path.Join(dir, "..", "RMSData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("RMS").UserConfig()
	}
