...
func GetGithubRelease(url, fallbackUrl string) (*GithubRelease, error) {
	...
	if res.StatusCode >= 300 {
		...
		// If that is the case, try our fallback at the GitHub API mirror
...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("RMS_DEV_INSTALL") == "1"
	...
	Log.Debug("Found existing RMS Install. Checking for hash...")
	scanner := bufio.NewScanner(f)
	if scanner.Scan() {
		line := scanner.Text()
		if strings.HasPrefix(line, "// RMS ") {
			InstalledHash = line[len("// RMS "):]
